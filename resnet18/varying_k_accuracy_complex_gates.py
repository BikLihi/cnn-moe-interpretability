...
from models.moe_layer.soft_gating_networks import SimpleGate, ConvGate
...
def main():
    ...
    # Load wandb data
    models_df = pd.read_csv('analysis/resnet18/model_paths_complex_gating.csv', sep=';')

    try:
        total_results = []
        
        for i in range(len(models_df)):
            model, moe_block = build_model(models_df.iloc[i]['Model'], models_df.iloc[i]['Constraint'], models_df.iloc[i]
        ...
        result_df.to_csv('analysis/resnet18/results/complex_gate_varying_k_accuracy_{}.csv'.format(constraint), index=True)
    except:
        print('Error for {} {} {}'.format(constraint))
