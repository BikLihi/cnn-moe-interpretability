...
class ResNet18MoE(BaseModel):
    def __init__(self, moe_layers, name='ResNet18MoE', lift_constraint=None):
        ...
        for moe_layer in moe_layers:
            if moe_layer.layer_position == 0:
                self.input_layer = moe_layer
            else:
    ...
    def forward(self, x, output_only=True):
        ...
        if type(self.input_layer) is MoeBlockLayer:
        ...
        for i, layer in enumerate(self.layers):
            if type(layer) is MoeBlockLayer or type(layer) is ResidualMoeBlockLayer:
