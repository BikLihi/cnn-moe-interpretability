...
class ResNet18SingleLayer(BaseModel):
    def __init__(self, moe_position, num_experts, top_k, gating_network, loss_fkt, w_aux=0.0, constr_threshold=0.0, name='ResNet18MoE'):
        ...
        if moe_position == 0:
            self.input_layer[0] = MoEConvLayer(num_experts, top_k, gating_network, loss_fkt, w_aux, constr_threshold, self.input_layer[0].in_channels, self.input_layer[0].out_channels,
                                            self.input_layer[0].kernel_size, self.input_layer[0].stride, self.input_layer[0].padding, self.input_layer[0].bias)

        else:
            self.layers[moe_position-1][1].conv2 = MoEConvLayer(num_experts, top_k, gating_network, loss_fkt, w_aux, constr_threshold, self.layers[moe_position-1][1].conv2.in_channels, self.layers[moe_position-1][1].conv2.out_channels,
                                            self.layers[moe_position-1][1].conv2.kernel_size, self.layers[moe_position-1][1].conv2.stride, self.layers[moe_position-1][1].conv2.padding, self.layers[moe_position-1][1].conv2.bias)
    ...
    def forward(self, x, output_only=True):
        ...
        if type(self.input_layer[0]) == MoEConvLayer:
            out = self.input_layer(x)
            aux_loss += self.input_layer[0].current_aux_loss
            examples_per_expert[0] = self.input_layer[0].current_examples_per_expert
            expert_importance[0] = self.input_layer[0].current_expert_importance
            weights = self.input_layer[0].current_weights
        ...
        for i, layer in enumerate(self.layers):
            if type(layer[1].conv2) == MoEConvLayer:
                out = layer(out)
                aux_loss += layer[1].conv2.current_aux_loss
                examples_per_expert[i+1] = layer[1].conv2.current_examples_per_expert
                expert_importance[i+1] = layer[1].conv2.current_expert_importance
                weights = layer[1].conv2.current_weights
            else:
                out = layer(out)
    ...
    def fit(
            ...
            for phase in ['training', 'validation']:
                ...
                for inputs, labels in dataloaders[phase]:
                    ...
                    with torch.set_grad_enabled(phase == 'training'):
                        ...
                        if 'aux_loss' in model_output:
                            ...
                            running_aux_loss += model_output['aux_loss'].item()
                ...
                if enable_logging:
                    for key in running_examples_per_expert:
                        ...
                        #         dataset_sizes[phase]
                        #     wandb.log({'Avg. Share of examples ' + phase + ' layer_' + str(key) +
                        #                '/expert_' + str(i): avg_examples.cpu()}, step=epoch)
                        for i, importance in enumerate(running_expert_importance[key]):
                            ...
                            wandb.log({'Avg. importance ' + phase + ' layer_' + str(key) +
                                       '/expert_' + str(i): avg_importance.cpu()}, step=epoch)
        ...
        if best_model and return_best_model:
