...
from models.moe_layer.resnet18.resnet18_basic_moe_block import ResnetMultiMoEBlock
...
class ResNet18MultiMoE(BaseModel):
    def __init__(self, moe_layer_positions, num_experts=None, top_k=None, loss_fkt=None, w_aux_loss=None, loss_function='importance', name='ResNet18MultiMoE', lift_constraint=None):
        ...
        for i, position in enumerate(moe_layer_positions):
            self.layers[position - 1] = ResnetMultiMoEBlock(position, num_experts=4, top_k=2, loss_fkt='importance', w_aux_loss=0.5)

        # Insert ResNet Layers
        for i in range(4):
    ...
    def forward(self, x, output_only=True):
        ...
        if type(self.input_layer) is ResnetMultiMoEBlock:
            moe_output = self.input_layer(x, output_only=False)
            aux_loss += moe_output['aux_loss']
            examples_per_expert[0] = moe_output['examples_per_expert']
            expert_importance[0] = moe_output['expert_importance']
        ...
        for i, layer in enumerate(self.layers):
            if type(layer) is ResnetMultiMoEBlock:
                moe_output = layer(out, output_only=False)
                aux_loss += moe_output['aux_loss']
                examples_per_expert[i+1] = moe_output['examples_per_expert']
                expert_importance[i+1] = moe_output['expert_importance']
                out = moe_output['output']
            else:
                out = layer(out)
    ...
    def fit(
        ...
        if enable_logging:
            ...
            if validation_data:
                wandb.config.update(
                    {'Num validation data': len(validation_data)})
        ...
        for epoch in range(num_epochs):
            ...
            for phase in ['training', 'validation']:
                ...
                for inputs, labels in dataloaders[phase]:
                    ...
                    with torch.set_grad_enabled(phase == 'training'):
                        ...
                        if 'aux_loss' in model_output:
                            ...
                            running_aux_loss += model_output['aux_loss'].detach().cpu().numpy()

                        # Update metrics and loss
                        running_loss += loss.item()
                        acc.update(model_output, labels)
                        examples_per_expert = model_output['examples_per_expert']
                        # for resnet_block in examples_per_expert:
                        #     for moe_block in resnet_block:
                        #         for cnn_layer in moe_block:

                        #     for layer in examples_per_expert[block]:
                        #         for i, expert in enumerate(layer):
                ...
                # if enable_logging:
                #     for block in running_examples_per_expert:
                #         # For each responding to ResNet Blocks
                #         for layer_num, layer_results in enumerate(running_examples_per_expert[block]):
                #             # For each BasicMoEBlocks in ResNet Block
                #             for conv_layer, number_examples in enumerate(layer_results):
                #                 # For each ConvMoELayer in BasicMoeBlock
                #                 avg_examples = np.array(number_examples) / float(dataset_sizes[phase])
                #                 for expert, avg in enumerate(avg_examples[0]):
                #                     wandb.log({'Avg. Share of examples ' + phase + '_block_' + str(block) +
                #                            '/layer_' + str(layer_num) + '/convLayer_' + str(conv_layer) + '/expert_' + str(expert): avg}, step=epoch)

                    # for i, importance in enumerate(running_expert_importance[key]):
                    #         avg_importance = importance / dataset_sizes[phase]
                    #         for i in len(avg_importance):
                    #             wandb.log({'Avg. importance ' + phase + ' layer_' + str(key) +
                    #                     + str(i + 1) + '/expert_' + str(i): avg_importance[i]}, step=epoch)
            ...
            if type(test_data) is dict:
                for key in test_data:
                    eval_result = self.evaluate(
                        test_data[key], batch_size, criterion, metrics, key)
                    wandb.config.update({
                        key + '_acc': eval_result['acc']
                    })
            else:
                eval_result = self.evaluate(
                    test_data, batch_size, criterion, metrics)
                wandb.config.update({
