...
class CocoDataset(Dataset):
    ...
    def __init__(self, root_dir, set_name='train2017', transform=None):
        ...
        if 'test' in set_name:
            self.coco      = COCO(os.path.join(self.root_dir, 'annotations', 'image_info_test-dev2017.json'))
        else:
...
class CocoSubDataset(CocoDataset):
    def __init__(self, root_dir, classes, set_name='train2017', transform=None):
        """
        Args:
            root_dir (string): COCO directory.
            transform (callable, optional): Optional transform to be applied
        ...
        self.image_ids = sorted(list(imgIds))
    ...
    def load_classes(self, classes):
        # load class names (name -> label)
        categories = self.coco.loadCats(self.coco.getCatIds(classes))
        categories.sort(key=lambda x: x['id'])
    ...
    def load_annotations(self, image_index):
        ...
        for idx, a in enumerate(coco_annotations):
            ...
            if a['category_id'] not in self.coco_labels_inverse:
                continue
