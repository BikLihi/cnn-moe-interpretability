...
from models.moe_layer.soft_gating_networks import SimpleGate, FCGate
...
# Variable parameters
loss_function_values = ['importance']
num_expert_values = [4]
top_k = 2
position_values = [4]
        ...
        for loss_function in loss_function_values:
            for i in range(3):
                    ...
                    model = ResNet18MoE(
                        ...
                        name='Residual_' + str(num_experts) + '_topK=' + str(top_k) + '_loss=' + loss_function + '_w_aux=' + str(w_aux_loss) + '_moePosition=' + str(position) + '_Noise=True_' + str(i),
                    ...
                    model.fit(
                        ...
                        wandb_name='Residual' + '_numExp=' + str(num_experts) + '_topK=' + str(top_k) + '_loss=' + loss_function + '_w_aux=' + str(w_aux_loss) + '_moePosition=' + str(position) + '_Noise=True_' + str(i),
                        wandb_tags=['MoE', 'ResNet18', gating_network.__name__, 'SoftConstraint', 'Residual'],
                    )
