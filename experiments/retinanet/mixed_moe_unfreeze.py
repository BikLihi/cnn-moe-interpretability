...
def main(args=None):
    ...
    enable_logging = True
    initial_lr = 1e-5
    project_name = 'MoE_mixed_overall_loss_w=0.25_SimpleGate'
    gating_network = SimpleGate

    # Create the data loaders
    class_file = 'data/coco/subset.names'
    classes = [line.strip() for line in open(class_file, 'r')]
    dataset_train = CocoSubDataset(coco_path, classes=classes, set_name='train2017',
                                transform=transforms.Compose([Normalizer(), Augmenter(), Resizer()]))
    dataset_val = CocoSubDataset(coco_path, classes=classes, set_name='val2017',
    ...
    retinanet.classificationModel = model.ClassificationModelMoE(num_features_in=256, num_experts=4, top_k=2, gating_network=gate1, num_classes=len(classes))
