...
def main(args=None):
    ...
    retinanet.classificationModel = model.ClassificationModelMoE(num_features_in=256, num_experts=4, top_k=2, gating_network=gate, num_classes=len(classes))
    ...
    if parser.enable_logging:
        wandb.init(project='RetinaNet', name='MoE_classification_overall_loss',
                    reinit=True,
                    config={
    ...
    for epoch_num in range(0, parser.epochs):
        ...
        result, running_examples_per_expert, running_expert_importance = coco_eval.evaluate_coco(dataset_val, retinanet)

        for i, number_examples in enumerate(running_examples_per_expert.tolist()):
            avg_examples = number_examples / (len(dataset_val) * 5)
            wandb.log({'Avg. Share of examples validation /expert_' + str(i): avg_examples}, step=total_iterations/1000)

        for i, importance in enumerate(running_expert_importance):
            avg_importance = importance / (len(dataset_val) * 5)
            wandb.log({'Avg. importance  validation /expert_' + str(i): avg_importance}, step=total_iterations/1000)
    ...
    with torch.no_grad():
        ...
        for j in range(idxs[0].shape[0]):
            ...
            box_data = {
                ...
                'box_caption': '{} {:1.3f} {}-{} {:.2f}'.format(label_name, float(score), int(sorted_weight_indices[0]), int(sorted_weight_indices[1]), weights[sorted_weight_indices[0]]),
                'scores': {'score': score} }
